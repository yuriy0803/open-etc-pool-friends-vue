<template>
  <div class="space-y-8" id="pool-dashboard-container">
    ...
    <div class="relative overflow-hidden rounded-2xl bg-gradient-to-b from-slate-900/95 via-[#0c121e] to-[#090d16] border border-slate-800/80 p-6 sm:p-8 shadow-2xl">
        ...
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          ...
          <div class="space-y-3 max-w-3xl">
            ...
            <h1 class="text-2xl sm:text-4xl lg:text-5xl font-extrabold tracking-tight text-white leading-tight">
              Live <span class="bg-gradient-to-r from-emerald-400 via-teal-300 to-cyan-400 bg-clip-text text-transparent">Mining Pool</span> Dashboard
            </h1>
          ...
          <div class="flex flex-row lg:flex-col items-end sm:items-center lg:items-end justify-between lg:justify-center gap-3">
            <button
              ...
              class="px-4 py-2.5 bg-slate-850 hover:bg-slate-800 border border-slate-700 hover:border-emerald-500/40 text-slate-200 hover:text-emerald-400 rounded-xl text-xs font-semibold transition-all flex items-center space-x-2 shadow-sm"
        ...
        <div class="mt-8 pt-6 border-t border-slate-800/80">
          ...
          <div v-if="dashboardData?.topMiners?.length" class="mt-3 flex items-center flex-wrap gap-2 text-xs text-slate-500">
            ...
            <button
              ...
              class="font-mono text-emerald-400 hover:text-emerald-300 hover:underline bg-slate-800/40 px-2 py-0.5 rounded border border-slate-700/40 transition-colors"
            >
    ...
    <div v-if="dashboardData?.hasErrors" class="p-4 rounded-xl bg-amber-500/10 border border-amber-500/20 text-amber-300 text-xs flex items-center justify-between">
      ...
      <button @click="refreshData" class="underline hover:text-white font-semibold ml-4">Retry</button>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4" id="pool-stat-cards">
      ...
      <StatCard
        ...
        :badgeClass="(dashboardData?.market?.change24hBtc || 0) >= 0 ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-rose-500/10 text-rose-400 border border-rose-500/20'"
        :icon="Coins"
      />
      <StatCard
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 bg-slate-900/60 p-4 rounded-2xl border border-slate-800/80 font-mono text-xs text-slate-300">
      <div class="p-3 bg-slate-950/60 rounded-xl border border-slate-800/60 flex flex-col justify-between space-y-1">
        <span class="text-[10px] uppercase tracking-wider text-slate-500">Est. Block Time</span>
        <div class="text-sm font-bold text-white">{{ dashboardData?.pool?.estBlockFindTime || 'Calculating...' }}</div>
      ...
      <div class="p-3 bg-slate-950/60 rounded-xl border border-slate-800/60 flex flex-col justify-between space-y-1">
        ...
        <div class="text-sm font-bold flex items-center space-x-1.5" :class="luckColorClass">
          ...
          <span class="text-[10px] font-sans font-normal text-slate-400">({{ luckLabel }})</span>
        </div>
        <div class="w-full bg-slate-800 h-1 rounded-full overflow-hidden mt-1">
          <div class="h-full bg-emerald-400 transition-all duration-500" :style="{ width: `${Math.min(100, dashboardData?.pool?.roundLuck || 100)}%` }"></div>
        </div>
      </div>

      <div class="p-3 bg-slate-950/60 rounded-xl border border-slate-800/60 flex flex-col justify-between space-y-1">
        <span class="text-[10px] uppercase tracking-wider text-slate-500">Matured Blocks</span>
        <div class="text-sm font-bold text-emerald-400">{{ dashboardData?.blocks?.maturedTotal || 0 }}</div>
      ...
      <div class="p-3 bg-slate-950/60 rounded-xl border border-slate-800/60 flex flex-col justify-between space-y-1">
        ...
        <div class="text-sm font-bold text-white">PPLNS (0.5%)</div>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-4 gap-6">
      ...
      <div class="xl:col-span-2 glass-card rounded-2xl p-5 sm:p-6 flex flex-col justify-between" id="pool-hashrate-chart-container">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
          <div>
            ...
            <p class="text-xs text-slate-400 mt-0.5">Continuous telemetry sampling from pool API</p>
          </div>
      ...
      <div class="glass-card rounded-2xl p-5 sm:p-6 flex flex-col justify-between">
        <div>
          ...
          <p class="text-xs text-slate-400 mt-0.5">Ethereum Classic ETCHASH network</p>
        </div>
        ...
        <div class="pt-4 border-t border-slate-800/60 text-xs text-slate-400 space-y-2 font-mono">
          <div class="flex justify-between">
            ...
            <span class="font-bold text-white">{{ dashboardData?.network?.formattedDifficulty || '0' }}</span>
          </div>
          <div class="flex justify-between">
            <span>Node Name:</span>
            <span class="font-bold text-slate-300">{{ dashboardData?.network?.nodeName || 'ETC Node' }}</span>
          </div>
        </div>
      </div>

      <!-- Stratum Ports & Quick Config (1 Col) -->
      <div class="glass-card rounded-2xl p-5 sm:p-6 flex flex-col justify-between space-y-5">
        <div>
          ...
          <p class="text-xs text-slate-400">Direct miner connection URLs</p>
        </div>
        ...
        <div class="space-y-3 font-mono text-xs">
          <div class="bg-slate-950/60 rounded-xl p-3 border border-slate-800/80">
            ...
            <div class="flex items-center justify-between text-emerald-400 font-semibold">
              ...
              <button @click="copyText('stratum+tcp://pool:8008')" class="ml-2 text-slate-400 hover:text-white" title="Copy TCP address">
          ...
          <div class="bg-slate-950/60 rounded-xl p-3 border border-slate-800/80">
            ...
            <div class="flex items-center justify-between text-teal-400 font-semibold">
              ...
              <button @click="copyText('stratum+ssl://pool:8443')" class="ml-2 text-slate-400 hover:text-white" title="Copy SSL address">
                <Copy class="w-3.5 h-3.5" />
              </button>
            </div>
          </div>
    ...
    <div class="glass-card rounded-2xl p-5 sm:p-6 space-y-4" id="pool-top-miners">
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-xs text-slate-400">Leading hashpower contributors currently connected</p>
        </div>
        <router-link to="/miners" class="text-xs text-emerald-400 hover:text-emerald-300 font-semibold flex items-center space-x-1">
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-left text-xs font-mono">
          ...
          <tbody class="divide-y divide-slate-800/60 text-slate-300">
            <tr v-for="(miner, idx) in dashboardData?.topMiners" :key="miner.address" class="hover:bg-slate-800/30 transition-colors">
              <td class="py-3 px-3 font-bold text-slate-500">
                #{{ idx + 1 }}
              </td>
              <td class="py-3 px-3 font-semibold text-white">
                <router-link :to="`/miner/${miner.address}`" class="hover:text-emerald-400 hover:underline">
              ...
              <td class="py-3 px-3 text-right font-sans">
                <router-link
                  ...
                  class="inline-flex items-center space-x-1 px-2.5 py-1 rounded-lg bg-slate-800 hover:bg-emerald-500/20 text-slate-300 hover:text-emerald-400 border border-slate-700/60 transition-colors text-[11px]"
                >
                  <span>Stats</span>
    ...
    <div class="glass-card rounded-2xl p-5 sm:p-6 space-y-4" id="pool-recent-blocks">
      <div class="flex items-center justify-between">
        <div>
          <h3 class="text-base font-bold text-white flex items-center space-x-2">
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-left text-xs font-mono">
          ...
          <tbody class="divide-y divide-slate-800/60 text-slate-300">
            <tr v-for="block in dashboardData?.blocks?.recentMatured" :key="block.height" class="hover:bg-slate-800/30 transition-colors">
              <td class="py-3 px-3 font-bold text-white">
                <a :href="`https://etc.blockscout.com/block/${block.height}`" target="_blank" rel="noopener" class="hover:text-emerald-400 underline decoration-slate-700">
                  #{{ block.height }}
                </a>
              </td>
              <td class="py-3 px-3 text-slate-400">
    ...
    <div class="glass-card rounded-2xl p-5 sm:p-6 space-y-4" id="pool-quick-estimator">
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p class="text-xs text-slate-400">Calculate projected ETC rewards based on current difficulty and market price</p>
        </div>
        ...
        <div class="flex items-center space-x-2">
          <input
            ...
            min="1"
            placeholder="100"
            class="bg-slate-950/80 border border-slate-700/80 focus:border-emerald-500 rounded-xl px-3 py-1.5 text-xs text-white font-mono w-28 focus:outline-none"
          />
          <select
            v-model="calcUnit"
            class="bg-slate-950/80 border border-slate-700/80 focus:border-emerald-500 rounded-xl px-2 py-1.5 text-xs text-slate-300 font-mono focus:outline-none"
          >
      ...
      <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 font-mono text-xs">
        <div class="bg-slate-950/50 p-3 rounded-xl border border-slate-800/80">
          ...
          <div class="text-base font-bold text-emerald-400 mt-1">{{ calculatedEstimate.dailyEtc }} ETC</div>
          <div class="text-[11px] text-slate-400 font-sans">≈ ${{ calculatedEstimate.dailyUsd }} USD</div>
        </div>

        <div class="bg-slate-950/50 p-3 rounded-xl border border-slate-800/80">
          <div class="text-[10px] text-slate-500 uppercase">Estimated 7 Days</div>
          <div class="text-base font-bold text-teal-400 mt-1">{{ calculatedEstimate.weeklyEtc }} ETC</div>
          <div class="text-[11px] text-slate-400 font-sans">≈ ${{ calculatedEstimate.weeklyUsd }} USD</div>
        </div>

        <div class="bg-slate-950/50 p-3 rounded-xl border border-slate-800/80">
          <div class="text-[10px] text-slate-500 uppercase">Estimated 30 Days</div>
          <div class="text-base font-bold text-cyan-400 mt-1">{{ calculatedEstimate.monthlyEtc }} ETC</div>
          <div class="text-[11px] text-slate-400 font-sans">≈ ${{ calculatedEstimate.monthlyUsd }} USD</div>
        </div>

        <div class="bg-slate-950/50 p-3 rounded-xl border border-slate-800/80 flex flex-col justify-between">
          <div class="text-[10px] text-slate-500 uppercase">Full Calculator</div>
          <router-link
            to="/calculator"
            class="text-xs text-emerald-400 hover:text-emerald-300 font-semibold flex items-center space-x-1 mt-2"
          >
