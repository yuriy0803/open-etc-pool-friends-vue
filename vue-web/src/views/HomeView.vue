<template>
  <div class="space-y-8">
    ...
    <div class="glass-card rounded-2xl p-5 sm:p-6 space-y-4" id="miner-payout-tracker">
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p class="text-xs text-slate-400">Fetch and inspect on-chain payout transactions for any ETC miner wallet</p>
        </div>
        ...
        <div class="flex items-center space-x-2">
          <input
            ...
            class="bg-slate-950/60 border border-slate-800 focus:border-emerald-500 rounded-xl px-3 py-2 text-xs text-white placeholder-slate-500 font-mono w-full sm:w-80 focus:outline-none transition-colors"
            @keyup.enter="fetchTrackerPayouts"
          />
          <button
            @click="fetchTrackerPayouts"
            :disabled="trackerLoading"
            class="px-4 py-2 bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-bold rounded-xl text-xs flex items-center space-x-1.5 transition-all shadow-sm flex-shrink-0"
          >
